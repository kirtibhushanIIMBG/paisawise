...
### 6.1a Presentation: dark by default, on a hairline grid

The brand palette above is unchanged and still drives the accent, so the deck and the site agree. What changed is the **ground it sits on**. The site's default presentation is dark, built on a near-black field with a hairline grid, following two reference templates the client supplied:

- **[Aspect](https://aspect-nextjs-template.vercel.app/)** is the primary reference. It is a fintech dashboard product, so its structure transfers directly: a near-black `#09090B` ground, two vertical rails at the content edges running the full page height, every section closing with a horizontal hairline so sections read as **cells in a grid** rather than floating cards, light-weight display headlines, and a bordered product frame beside the hero copy. Its footer carries a regulatory-disclaimer block, which is exactly where the academic-project notice belongs.
- **[CryptGen](https://cryptgen-template-aceternity.vercel.app/)** supplies the hero treatments: display text that fades from solid to recessive toward the bottom-right, pill buttons with a top-edge highlight, a radial violet bloom, and a perspective grid floor behind the closing panel.

Additional tokens for the dark ground, named for what they are rather than by any framework scale:

| Token | Hex | Use |
|---|---|---|
| `void` | `#09090B` | page ground |
| `void-2` | `#0E0E12` | cell fill |
| `void-3` | `#131318` | raised cell |
| `hairline` | `#212127` | **the grid line**, carries most of the layout |
| `hairline-2` | `#2C2C34` | stronger edge, button borders |
| `quiet` | `#A1A1AA` | body copy on dark |
| `faint` | `#71717A` | secondary text on dark |
| `mint-l` | `#34D399` | positives on dark |

Two rules that follow from this:

1. **Components never name a brand colour for a surface or for text.** They use the semantic layer (`bg-bg`, `bg-panel`, `text-fg`, `text-copy`, `text-dim`, `text-accent`, `text-positive`, `border-edge`). That layer is what flips between themes, and it is the reason light mode keeps working for free when the default changes.
2. **Light mode remains available** behind the header toggle. `dark` ships on the server-rendered `<html>` tag and the inline theme script only ever *removes* it, so the default survives with JavaScript off and there is no flash in either direction.

### 6.1b Chart steps must be re-derived for the dark ground, not flipped

Dark chart colours are **selected against the surface they sit on**, never generated by inverting the light set. Run `dataviz`'s `scripts/validate_palette.js` against the actual cell fill before committing any step. On this ground the steps are:

| Token | Hex | Contrast vs `#0E0E12` |
|---|---|---|
| `plot` | `#8B74F0` | 5.37:1 |
| `plot-soft` | `#4A4080` | 2.14:1 (clears the 2.0 ordinal floor) |
| `plot-ref` | `#7C7C8A` | 4.68:1 |

The category bars are one hue in two steps, sorted by size, because spending categories are **nominal**: a value ramp across them would double-encode bar length as colour. Identity comes from the axis labels. Over-budget state is carried by an icon plus a written amount, never by colour alone.
